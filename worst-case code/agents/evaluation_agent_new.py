...
from core.context_builder_withemo import json_to_dialogue_str_with_emotion
from prompts.evaluation_prompt_new import evaluation_prompt
# chat_eva需要重写
...
def evaluation_agent(profile_path: str, dialogue_path: str) -> Dict[str, Any]:
    ...
    返回：
    {
        "analysis": "...",
        "evaluation": {
            "deep emotional understanding": 1-5,
            "exploratory guidance": 1-5,
            "balanced emotional support": 1-5,
            "authentic and grounded support": 1-5
        }
    }
    """
    ...
    chat_history = json_to_dialogue_str_with_emotion(dialogue_path)
    ...
    if not isinstance(evaluation, dict):
        raise ValueError(f"❌ 'evaluation' 字段不是字典：{evaluation}")
